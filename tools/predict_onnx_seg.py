...
class YOLOSeg:
    ...
    def __call__(self, im0, nm=32):
        ...
        boxes, segments, masks = self.postprocess(
            ...
            pad_w=pad_w,
            pad_h=pad_h,
            conf_threshold=self.confidence_thres,
...
if __name__ == "__main__":
    ...
    detection = YOLOSeg(args.model, args.score_thr, 0.5,imgsz=imgsz,names=names)
    ...
    for f in files:
        img = cv2.imread(f)
        boxes, segments,_ = detection(img)
        detection.draw_and_visualize(img, boxes, segments, vis=False, save=True)
